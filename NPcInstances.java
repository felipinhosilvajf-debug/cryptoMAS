...
import l2f.gameserver.model.actor.listener.NpcListenerList;
import l2f.gameserver.model.actor.recorder.NpcStatsChangeRecorder;
...
import l2f.gameserver.network.serverpackets.AcquireSkillDone;
import l2f.gameserver.network.serverpackets.AcquireSkillList;
import l2f.gameserver.network.serverpackets.ActionFail;
import l2f.gameserver.network.serverpackets.AutoAttackStart;
import l2f.gameserver.network.serverpackets.ExChangeNpcState;
...
import l2f.gameserver.stats.funcs.FuncMonsterBalancer;
...
import l2f.gameserver.utils.CertificationFunctions;
...
public class NpcInstance extends Creature
{
	...
	//temp change from private to public
...
//		}
	...
	public void showChatWindow(Player player, int val, Object... replace)
	{
		...
		// [SISTEMA GLOBAL DE IMPOSTOS E TERRITÓRIOS - ADICIONADO]
		l2f.gameserver.model.entity.residence.Castle castle = getCastle();
		if (castle != null)
		{
			l2f.gameserver.model.pledge.Clan owner = castle.getOwner();
			String clanName = (owner != null) ? owner.getName() : "Sem Clan";
			String leaderName = (owner != null) ? owner.getLeaderName() : "Nenhum";

			packet.replace("%castlename%", castle.getName());
			packet.replace("%clanname%", clanName);
			packet.replace("%clanleadername%", leaderName);
			// Puxa o número inteiro da tabela castle (ex: 15) e adiciona o símbolo de %
			packet.replace("%taxpercent%", String.valueOf(castle.getTaxPercent()) + "%");
		}
		else
		{
			// Fallback de segurança caso o NPC esteja fora de zonas mapeadas de castelo
			packet.replace("%castlename%", "Open");
			packet.replace("%clanname%", "Nenhum");
			packet.replace("%clanleadername%", "Nenhum");
			packet.replace("%taxpercent%", "0%");
		}
	...
	public void showSkillList(Player player)
	{
		...
		if (!(getTemplate().canTeach(classId) || getTemplate().canTeach(classId.getParent(player.getSex()))))
		{
			...
			else
			{
				...
				StringBuilder sb = new StringBuilder();
				sb.append("<html><head><body>");
				sb.append(new CustomMessage("l2f.gameserver.model.instances.L2NpcInstance.WrongTeacherClass", player));
				sb.append("</body></html>");
				html.setHtml(sb.toString());
				player.sendPacket(html);
			}
